function [CVstats, CVstats_mu, CVstats_std, h0, t0] = KfoldCrossValCCA_VA(X,Y,Ymu,Ysigma,dp,dt,dcca,k,verbal,weights)
...
% KfoldCrossValCCA version Variance Adjust (VA): 
% inflates the variance to match the observations when computing cross val - updated Mar2020
% (functionally needed only for generating the uncertainty intervals in the final reconstruction via RMSEP of CrossVal)
...
for i = 1:size(indexVal,2)
    ...
    [CVstats.R2Calib(i,:), CVstats.r2Verif(i,:), CVstats.RE(i,:), CVstats.CE(i,:), CVstats.RMSEP(i,:)] =... % (m x A x k), where k is the CV iteration
        CCA_CrossVal_R2RECE_varAdj(X,Y,Ymu,Ysigma,B,indexVal(:,i),indexCal(:,i)); % each row represents a new cross-validatio (k), each column a different mode (A)
end
...
% =========================================================================
function [R2Cal,r2Val,RE,CE,RMSEP] = CCA_CrossVal_R2RECE_varAdj(X,Y,Ymu,Ysigma,B,indexVal,indexCal)
...
% =========================================================================
    ...
    YCalib = X(indexCal,:)*B' .* Ysigma + Ymu; 
        % <--- added code to inflate variance!  10/31/19 (BOO!)
        YValid_mean = nanmean(YValid,1);
        YCalib_mean = nanmean(YCalib,1);
        YValid = (YValid -   YValid_mean) .* std(Ytrue,1,1)./std(YValid,1,1)  + YValid_mean;
        YCalib = (YCalib -   YCalib_mean) .* std(Ytrue,1,1)./std(YCalib,1,1)  + YCalib_mean;        
